...
async function loadModel(){
  ...
  return (await predict(imagePath) + " potholes detected!");
}


// Preprocess the image/frame
async function preprocessImage(imagePath) {
  const resizedImage = await sharp(imagePath).resize(640, 640).toBuffer();
...
async function predict(imagePath) {
  ...
  const metaData = await img.metadata();  // to extract the original metadata of input image 
  const [imgWidth, imgHeight] = [metaData.width, metaData.height];  // width and height of input image
