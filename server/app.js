...
app.post("/addtoDB", async (req, res) => {
  ...
  const stringUrl = `https://geocode.maps.co/reverse?lat=${latitude}&lon=${longitude}&api_key=${process.env.API_KEY}`;

  const address = await fetch(stringUrl);
  ...
  const newPotholeEntry = new Pothole({
    latitude,
    longitude,
    numberOfPotholes: coordinates.length,
